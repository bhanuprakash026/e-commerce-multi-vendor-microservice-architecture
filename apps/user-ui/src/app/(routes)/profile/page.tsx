      ...
      <div className='md:max-w-7xl mx-auto'>
        ...
        <div className="text-center mb-10">
          <h1 className='text-3xl font-bold text-gray-800'>Welcome back,
            <span className='text-blue-600'>{isLoading ? (
        ...
        <div className="mt-10 flex flex-col md:flex-row gap-6">
          ...
          <div className="bg-white p-6 rounded-md shadow-sm border border-gray-100 w-full md:w-[55%]">
            <h2 className='text-xl font-medium text-gray-800 mb-4'>{activeTab}</h2>
            {activeTab === "Profile" && !isLoading && user ? (
              <div className="space-y-4 text-sm text-gray-700">
                <div className="flex items-center gap-3">
                  <Image className='w-16 h-16 rounded-full border border-gray-200' src={user?.avatar ?? "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSPrTBPxjgQtxbR_H3BQ_QhM0DVz9eaHSoVv-WGiklBOS2X4heHr1WqAawX2RTqv2J2SNI&usqp=CAU"} alt="" width={60} height={60} />
                  <button
...
const NavItem = ({ label, Icon, active, danger, onClick }: any) => {
  return <button
    onClick={onClick}
    className={`w-full flex items-center gap-2 px-3 py-3 rounded-md text-sm font-medium transition ${active ?
