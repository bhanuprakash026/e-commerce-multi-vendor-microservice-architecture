...
export const trackOtpRequests = async (email: string, next: NextFunction) => {
    // How can we understand that how many times you send the OTPs
...
export const sendOtp = async (name: string, email: string, template: string) => {
    ...
    await sendeMail(email, { name, otp }, template, "Verify Your Email");
    await redis.set(`otp:${email}`, otp, "EX", 300) // OTP will expires after 5 min. Here 300 is seconds
    await redis.set(`otp_cooldown:${email}`, "true", "EX", 60) // We can only able to send the otp to email after 1 min. 
};
