...
import DeleteDiscountCodeModal from '@/shared/modules/auth/components/modals/delete.discount-codes'

const Page = () => {
  const [showModal, setShowModal] = useState(false);
  const [showDeleteModal, setShowDeleteModal] = useState(false);
  const [selectedDeleteDiscountCode, setSelectedDeleteDiscountCode] = useState<any>(null);
  ...
  const handleDeleteClick = async (discount: any) => {
    ...
    setSelectedDeleteDiscountCode(discount);
  };
  ...
  return (
    <div className="w-full min-h-screen p-8">
      ...
      <div className="flex items-center text-white">
        ...
        <span>Discount Codess</span>
      </div>

      <div className="mt-8 bg-gray-900 p-6 rounded-lg shodow-lg">
        ...
        <div className='fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex items-center justify-center'>
          <div className='bg-gray-800  p-6 rounded-lg w-[450px] shadow-lg'>
            <div className='flex justify-between items-center border-b border-gray-700 pb-3'>
              <h3 className='text-white'>Create Discount code</h3>
              <button
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="m-4">
              ...
              {createDiscountCodeMutation.isError && (
                <p>
                  ...
                  )?.response?.data?.message || "Somehing went wrong"}
                </p>
              )}
            </form>
          </div>
      ...
      { 
        showDeleteModal && selectedDeleteDiscountCode && (
          <DeleteDiscountCodeModal 
            discount={selectedDeleteDiscountCode}
            onClose={() => { setShowDeleteModal(false) }}
            onConfirm={() => deleteDiscountCodeMutation.mutate(selectedDeleteDiscountCode?.id)}
          />
        )
      } 
