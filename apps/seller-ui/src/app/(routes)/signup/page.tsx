...
const Signup = () => {
    ...
    const verifyOtpMutation = useMutation({
        mutationFn: async () => {
            const response = await axios.post(`http://localhost:8080/api/verify-seller`, { ...sellerData, otp: otp.join("") });
            return response.data;
        },
        onSuccess: (data) => {
    ...
    return (
        <div className="w-full flex flex-col items-center pt-10 min-h-screen">
            ...
            {/* Steps container */}
            <div className='md:w-[480px] p-8 bg-white shadow rounded-lg border border-gray-200'>
                {activeStep === 1 && (
                    <>
                        {!showOtp ? (
